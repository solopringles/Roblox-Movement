	...
	Abilities = {
		Active1 = {
			...
			ExecuteServer = function(player, character)
				...
				MovementUtil.ApplyVelocity(hrp, dashDir * 150, 0.4)
					...
					while tick() - start < 0.4 do
						...
						for _, part in pairs(parts) do
							...
							if m and m:FindFirstChild("Humanoid") and m ~= character then
								MovementUtil.ApplyKnockback(m, dashDir + Vector3.new(0,0.5,0), 100)
							end
						end
						task.wait(0.05)
					end
		...
		Active2 = {
			...
			ExecuteServer = function(player, character)
				...
				-- Weighted pull down
				MovementUtil.CreateExplosionPush(hrp.Position, 30, -700000, {character})
